    ...
    for fid, blk in sorted(blocks.items(), key=lambda x: x[1]["fid_num"]):
        ...
        for cand_name, cand_idx in candidates:
            ...
            while True:
                ...
                try:
                    name0 = (
                        data[meta_start + 63 : meta_start + 63 + 80]
                        .decode("utf-16-le")
                        .split("\x00")[0]
                    )
                except (UnicodeDecodeError, IndexError):
                ...
                if (
                    name0 != ddi_first
                    and not name0.startswith(ddi_first)
                    and not ddi_first.startswith(name0)
                ):
                    continue
                ...
                if actual_slots >= nvars:
                    ...
                    try:
                        name_last = (
                            data[last_slot + 63 : last_slot + 63 + 80]
                            .decode("utf-16-le")
                            .split("\x00")[0]
                        )
                ...
                if not last_ok and actual_slots > 0 and abs(actual_slots - nvars) <= 3:
                    ...
                    try:
                        name_last_a = (
                            data[last_slot_actual + 63 : last_slot_actual + 63 + 80]
                            .decode("utf-16-le")
                            .split("\x00")[0]
                        )
...
def extract_block(
    data: bytes, block_info: dict, merged_vars: list, meta_start: int
) -> pd.DataFrame:
    ...
    if data_start < 8:
        offset_vars = [
            (i, v)
            for i, v in enumerate(merged_vars)
...
def convert_nesstar(
    ...
    if verbose:
        ...
        for fid, blk in sorted(blocks.items(), key=lambda x: x[1]["fid_num"]):
            print(
                f"    {fid:>4s}  {blk['name'][:55]:55s}  {blk['nrecs']:>10,} records  "
...
def _write_excel(df: pd.DataFrame, out_path: str, blk: dict, merged_vars: list):
...
def _write_stata(df: pd.DataFrame, out_path: str, blk: dict, merged_vars: list):
...
def show_info(nesstar_path: str, ddi_path: str):
    ...
    for fid, blk in sorted(blocks.items(), key=lambda x: x[1]["fid_num"]):
        ...
        print(
            f"  {fid:>4s}  {blk['name'][:55]:<55s}  {blk['nrecs']:>10,}  "
    ...
    for v in first_blk["ddi_vars"][:10]:
        ...
        print(
            f"    {v['name']:20s}  {v['type']:10s}  w={v['ddi_width']:>3}  "
...
def main():
    ...
    batch_p.add_argument(
        "--survey", "-s", default="hces", help="Survey name (directory under data/)"
    )
    batch_p.add_argument(
        "--formats",
        "-f",
    ...
    if args.command == "info":
        ...
        fmt_list = (
            ALL_FORMATS
            if args.formats == "all"
            else [f.strip().lower() for f in args.formats.split(",")]
        )
