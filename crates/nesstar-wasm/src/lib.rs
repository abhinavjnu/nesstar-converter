...
pub fn convert_nesstar(nesstar_bytes: &[u8], ddi_xml: &str, format: &str) -> Result<js_sys::Uint8Array, JsValue> {
    ...
    if let Ok(layout) = is_resource {
        let headers: Vec<String> = layout.columns.iter().map(|c| c.variable.name.clone()).collect();
        
        if !is_json && !is_jsonl {
            let sep = if is_tsv { b"\t" } else { b"," };
            for (idx, h) in headers.iter().enumerate() {
                if idx > 0 { output_bytes.extend_from_slice(sep); }
        ...
        let layout = discover_metadata_layout(source.bytes(), block)
            .map_err(|e| JsValue::from_str(&format!("Layout Error: {e}")))?;
        let headers: Vec<String> = layout.columns_in_ddi_order().into_iter().map(|c| c.variable.name.clone()).collect();
