...
pub fn get_dataset_info(_nesstar_bytes: &[u8], ddi_xml: &str) -> Result<JsValue, JsValue> {
    ...
    let block = metadata.blocks.first()
        .ok_or_else(|| JsValue::from_str("DDI XML contains no data blocks"))?;

    let cols = block.variables.iter().map(|v| ColumnInfo {
        name: v.name.clone(),
        label: v.label.clone(),
...
#[wasm_bindgen]
pub fn preview_nesstar(nesstar_bytes: &[u8], ddi_xml: &str, limit: usize) -> Result<JsValue, JsValue> {
    let metadata = parse_ddi_reader(ddi_xml.as_bytes())
        .map_err(|e| JsValue::from_str(&format!("DDI Error: {e}")))?;
    let block = metadata.blocks.first()
        .ok_or_else(|| JsValue::from_str("DDI XML contains no data blocks"))?;
    ...
    if let Ok(layout) = discover_resource_layout(source.bytes(), block) {
        headers = layout.columns.iter().map(|c| c.variable.name.clone()).collect();
        let _ = decode_resource_batches(&source, &layout, max_rows, || true, |batch| {
        ...
        headers = layout.columns_in_ddi_order().into_iter().map(|c| c.variable.name.clone()).collect();
        let _ = decode_metadata_batches(&source, &layout, max_rows, || true, |batch| {
            for row_idx in 0..batch.row_count.min(max_rows - rows.len()) {
                let mut row = Vec::with_capacity(batch.columns.len());
                for col in &batch.columns {
                    match &col.values[row_idx] {
...
#[wasm_bindgen]
pub fn convert_nesstar(nesstar_bytes: &[u8], ddi_xml: &str, format: &str) -> Result<js_sys::Uint8Array, JsValue> {
    let metadata = parse_ddi_reader(ddi_xml.as_bytes())
        .map_err(|e| JsValue::from_str(&format!("DDI Error: {e}")))?;
    let block = metadata.blocks.first()
        .ok_or_else(|| JsValue::from_str("DDI XML contains no data blocks"))?;
    ...
    let (headers, variables): (Vec<String>, Vec<VariableDefinition>) = match &is_resource {
        ...
        Err(_) => {
            ...
            (
                ...
                layout.columns_in_ddi_order().into_iter().map(|c| c.variable.clone()).collect(),
            )
        }
    };

    let fmt = format.trim().to_lowercase();
    let result = convert_to_format_bytes(&source, block, &headers, &variables, is_resource.is_ok(), &fmt)?;
...
fn convert_to_format_bytes(
    ...
    match format {
        "parquet" => {
            ...
            if is_resource {
                let layout = discover_resource_layout(source.bytes(), block)
                    .map_err(|e| JsValue::from_str(&e.to_string()))?;
                decode_resource_batches(source, &layout, 5_000, || true, |batch| {
                    writer.write_batch(&batch).map_err(DecodeError::Writer)
                }).map_err(|e| JsValue::from_str(&e.to_string()))?;
            } else {
                let layout = discover_metadata_layout(source.bytes(), block)
                    .map_err(|e| JsValue::from_str(&e.to_string()))?;
                decode_metadata_batches(source, &layout, 5_000, || true, |batch| {
                    writer.write_batch(&batch).map_err(DecodeError::Writer)
                }).map_err(|e| JsValue::from_str(&e.to_string()))?;
            }
        ...
        "dta" | "stata" => {
            ...
            if is_resource {
                let layout = discover_resource_layout(source.bytes(), block)
                    .map_err(|e| JsValue::from_str(&e.to_string()))?;
                decode_resource_batches(source, &layout, 5_000, || true, |batch| {
                    writer.write_batch(&batch, variables).map_err(DecodeError::Writer)
                }).map_err(|e| JsValue::from_str(&e.to_string()))?;
            } else {
                let layout = discover_metadata_layout(source.bytes(), block)
                    .map_err(|e| JsValue::from_str(&e.to_string()))?;
            ...
            let cursor = writer.finish(headers, variables)
        ...
        "sav" | "spss" => {
            ...
            let cursor = writer.finish(variables)
                .map_err(|e| JsValue::from_str(&e))?;
            Ok(cursor.into_inner())
        }
        "fwf" => {
            ...
            if is_resource {
                let layout = discover_resource_layout(source.bytes(), block)
                    .map_err(|e| JsValue::from_str(&e.to_string()))?;
                decode_resource_batches(source, &layout, 5_000, || true, |batch| {
                    writer.write_batch(&batch).map_err(|e| DecodeError::Writer(e.to_string()))
                }).map_err(|e| JsValue::from_str(&e.to_string()))?;
            } else {
                let layout = discover_metadata_layout(source.bytes(), block)
                    .map_err(|e| JsValue::from_str(&e.to_string()))?;
                ...
                decode_resource_batches(source, &layout, 5_000, || true, |batch| {
                    for row_idx in 0..batch.row_count {
                        ...
                        for (c_idx, col) in batch.columns.iter().enumerate() {
                            match &col.values[row_idx] {
                                CellValue::Missing => { obj.insert(headers[c_idx].clone(), serde_json::Value::Null); }
                                CellValue::Text(s) => { obj.insert(headers[c_idx].clone(), serde_json::Value::String(s.clone())); }
                            }
                        }
        ...
        _ => {
            ...
            for (idx, h) in headers.iter().enumerate() {
                if idx > 0 { output_bytes.push(sep); }
            ...
            if is_resource {
                ...
                decode_resource_batches(source, &layout, 5_000, || true, |batch| {
                    for row_idx in 0..batch.row_count {
                        for (c_idx, col) in batch.columns.iter().enumerate() {
                            if c_idx > 0 { output_bytes.push(sep); }
                            match &col.values[row_idx] {
                                CellValue::Missing => {}
                                CellValue::Text(s) => {
                                    if !is_tsv && (s.contains(',') || s.contains('"') || s.contains('\n')) {
                                        output_bytes.push(b'"');
                                        output_bytes.extend_from_slice(s.replace('"', "\"\"").as_bytes());
                                        output_bytes.push(b'"');
                                    } else {
